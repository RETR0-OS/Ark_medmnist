...
from albumentations import (
    Compose, HorizontalFlip, CLAHE, HueSaturationValue, RandomBrightnessContrast, RandomGamma,OneOf,
    ToFloat, ShiftScaleRotate,GridDistortion, ElasticTransform, HueSaturationValue,
    RGBShift, Blur, MotionBlur, MedianBlur, GaussNoise,CenterCrop, GaussNoise,OpticalDistortion,RandomSizedCrop, RandomResizedCrop, Normalize
)
...
class PathMNIST(Dataset):
    def __init__(self, images_path, file_path, augment, num_class=9, annotation_percent=100):
        self.img_list = []  # list of image paths
        self.img_label = []  # list of image labels
        self.augment = augment  # augmentation functions to apply
        self.train_augment = build_ts_transformations()  # training augmentations
...
class OrganAMNIST(Dataset):
    def __init__(self, images_path, file_path, augment, num_class=11, annotation_percent=100):
        self.img_list = []  # list of image paths
        self.img_label = []  # list of image labels
        self.augment = augment  # augmentation functions to apply
        self.train_augment = build_ts_transformations()  # training augmentations
